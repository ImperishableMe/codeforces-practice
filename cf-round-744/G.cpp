...
void solve(int t){
	...
	int inf = 1e9;
	vector<vector<int>> dp(2, vector<int> (2005, inf));
	dp[0][0] = 0;
	for (int i = 0; i < n; i++){
		fill(all(dp[(i+1)&1]), inf);
		for (int l = 0; l <= 2000; l++){
			dp[(i+1)&1][max(0,l-a[i+1])] = min(dp[i&1][l] + a[i+1], dp[(i+1)&1][max(0, l-a[i+1])]);

			if (l + a[i+1] <= 2000) 
				dp[(i+1)&1][l+a[i+1]] = min(max(0,dp[i&1][l] - a[i+1]), dp[(i+1)&1][l+a[i+1]]);
		}
	}

	int ans = inf;
	for (int l = 0; l <= 2000; l++){
		ans = min(ans, dp[n&1][l] + l);
	}
	cout << ans << "\n";
}
int main(){
